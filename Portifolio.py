...
st.set_page_config(
    ...
    page_icon="logo.png",  # Você pode usar um ícone emoji ou uma URL de ícone
...
data = pd.read_csv('banco_transformado.csv')
...
if st.session_state["authentication_status"]:
    ...
    if 'client_code' not in client_info or not client_info['client_code']:
        ...
        if st.button("Salvar"):
            ...
            if client_code in data['Fornecedor'].values or client_code == admin_code:
        ...
        if role == 'admin': #Bloco de dashboard do administrador
            ...
            enterprise_options = ["Todos"] + list(data['Fornecedor'].unique())
            ...
            # Filtrar os Equipamentoss com base no cliente selecionado
            if st.session_state.selected_enterprise == "Todos":
                filtered_data = data
            else:
                filtered_data = data[data['Fornecedor'] == st.session_state.selected_enterprise]
            
            # Segundo filtro: Equipamentos
            model_options = ["Todos"] + list(filtered_data['Equipamentos'].unique())
            st.session_state.selected_model = st.sidebar.selectbox(
                "Selecione um Equipamentos de equipamento",
                options=model_options,
                index=model_options.index(st.session_state.selected_model)
            )
            
            # Filtrar os números de série com base no Equipamentos selecionado
            if st.session_state.selected_model == "Todos":
                filtered_data = filtered_data
            else:
                filtered_data = filtered_data[filtered_data['Equipamentos'] == st.session_state.selected_model]
            
            # Terceiro filtro: Número de Série
            serial_options = ["Todos"] + list(filtered_data['Numero_Serial'].unique())
            ...
            if st.session_state.selected_serial == "Todos":
                ...
                filtered_data = filtered_data[filtered_data['Numero_Serial'] == st.session_state.selected_serial]
            ...
            with cliente_container:
                ...
                A4Pb = df_selection['Manutenção_Preventiva'].sum()
                A4Cor = df_selection['Manutenção_Corretiva'].sum()
                A3Pb = df_selection['Manutenção_Emergencial'].sum()
                A3Cor = df_selection['Manutenção preditiva'].sum()
                total = df_selection['total'].sum()
                equipamentos = df_selection['Numero_Serial']
                cores = ['#1f77b4', '#ff7f0e']
                ...
                df_grouped_total = df_selection.groupby('Numero_Serial', as_index=False).sum(numeric_only=True)
                bar1 = px.bar(df_grouped_total, y='total', x='Numero_Serial', title='Total por Equipamento', color='Numero_Serial')
                ...
                filtered_data['Producao_Cor'] = filtered_data['Manutenção_Corretiva'] + filtered_data['Manutenção preditiva']
                filtered_data['Producao_PB'] = filtered_data['Manutenção_Preventiva'] + filtered_data['Manutenção_Emergencial']
                ...
                resumo = df_selection.groupby(df_selection['Numero_Serial']).agg({
                'Equipamentos': 'first',
                'Manutenção_Preventiva': 'sum',
                'Manutenção_Emergencial': 'sum',
                'Manutenção_Corretiva': 'sum',
                'Manutenção preditiva': 'sum',
                'total': 'sum'
                ...
                resumo = resumo.rename(columns={
                    ...
                    'Numero_Serial': 'Série',
                    'Manutenção_Preventiva': 'P&B A4',
                    'Manutenção_Emergencial': 'P&B A3',
                    'Manutenção_Corretiva': 'COR A4',
                    'Manutenção preditiva': 'COR A3',
                    'total': 'TOTAL'
                })
            
                
                coluna_ignorada = 'Equipamentos', 'Série'
                ...
                nova_ordem = ['Equipamentos','Série','P&B A4','COR A4','P&B A3','COR A3','TOTAL']
                ...
                resumo_total ={ 'P&B A4':df_selection['Manutenção_Preventiva'].sum(), 
                            'P&B A3':df_selection['Manutenção_Emergencial'].sum(), 
                            'COR A4':df_selection['Manutenção_Corretiva'].sum(), 
                            'COR A3':df_selection['Manutenção preditiva'].sum(), 
                            'TOTAL':df_selection['total'].sum()}
            ...
            # Primeiro filtro: Cliente
            enterprise_names = data[data['Fornecedor'] == config['credentials']['usernames'][client_id]['client_code']]['Fornecedor']
        ...
        <div class="footer">
            <p>© 2024 Desenvolvido por Thiago Medeiros.</p>
        </div>
        """
